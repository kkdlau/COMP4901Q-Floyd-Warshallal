...
#define for_each_block                                                         \
  for (int r = 0; r < NUM_BLK_PER_ROWS; r++)                                   \
    for (int c = 0; c < NUM_BLK_PER_ROWS; c++)
...
void MPI_floydWarshall(InlineMatrix input_inlined, SMatrix input_matrix,
                       InlineMatrix output_inlined, SMatrix output_matrix);

void copy_block(SMatrix matrix, int r, int c, int block_width,
                InlineMatrix dist);

void block_wise_min(int *dist, int *A_ik, int *A_kj, const int block_width,
                    const int k);
...
int main(int argc, char **argv) {
  ...
  if (!is_master) {
    malloc_matrix(SIZE_N, input_matrix, input_inlined);
  }
  ...
  MPI_floydWarshall(input_inlined, input_matrix, output_inlined, output_matrix);
  ...
  if (RANK == 0) {
    ...
    cout << "Floyd Warshall openMP Runtime: " << (p_end - p_start) << "s"
         << endl;
  }
...
void MPI_floydWarshall(InlineMatrix input_inlined, SMatrix input_matrix,
  ...
  if (is_master) {
    for_each_block {
      printf("master at %d, %d\n", r, c);
      int *send_buf = new int[BWIDTH * BWIDTH];
      send_bufs.push_back(send_buf);
      copy_block(input_matrix, r, c, BWIDTH, send_buf);
      MPI_Isend(send_buf, BSIZE, MPI_INT, r * BWIDTH + c, 0, MPI_COMM_WORLD,
                &req);
    }
  }
  MPI_Recv(self_buf, BSIZE, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  MPI_Barrier(MPI_COMM_WORLD);
  write_matrix(input_matrix, row_rank, col_rank, BWIDTH, self_buf);
  // printf("from [%d, %d]\n", row_rank, col_rank);s
  ...
  for (int k = 0; k < SIZE_N; k++) {
    if (is_master) {
      printf("at k = %d, %d, index = %d\n", k, BWIDTH, (int)(k / BWIDTH));
    }
    // column-group send
    ...
    //   printf("from %d, %d:\n", row_rank, col_rank);
    //   printf("A[%d, %d] -> cb_buf:\n", row_rank, (int)(k / BWIDTH));
    //   arr_print(cb_buf, BWIDTH);
    //   printf("A[%d, %d] -> rb_buf:\n", (int)(k / BWIDTH), col_rank);
    //   arr_print(rb_buf, BWIDTH);
    // }
    ...
    if (is_master) {
      ...
      for_each_block {
        MPI_Recv(recv_buf, BSIZE, MPI_INT, r * BWIDTH + c, 0, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
        write_matrix(input_matrix, r, c, BWIDTH, recv_buf);
      }
    }
    MPI_Barrier(MPI_COMM_WORLD);
  }
